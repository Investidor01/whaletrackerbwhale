...
import { TIMEFRAMES, tfSeconds } from "@/lib/binance";
...
export function DashboardView({ pairs, title, subtitle, icon = "🐋", priceDigits = 2 }: Props) {
  ...
  // Ensure the chosen pair belongs to this view's allowed list.
  useEffect(() => {
    if (!pairs.includes(config.pair)) setConfig({ pair: pairs[0] });
  }, [pairs, config.pair, setConfig]);
  ...
  return (
    <div className="flex flex-col gap-3 animate-fade-up">
      ...
      <div className="binance-panel rounded-lg p-3">
        <div className="flex items-start justify-between gap-3">
          ...
          <div className="text-right">
            <div className={`font-display text-2xl font-bold ${change >= 0 ? "text-[color:var(--win)]" : "text-[color:var(--loss)]"}`}>{last ? last.close.toFixed(priceDigits) : "—"}</div>
      ...
      <div className="grid grid-cols-2 gap-2">
        <Stat label="Preço" value={last ? last.close.toFixed(priceDigits) : "—"} sub={`${change >= 0 ? "+" : ""}${change.toFixed(2)}%`} positive={change >= 0} />
        <Stat label="Assertividade" value={`${stats.acc}%`} sub={`${stats.wins}W / ${stats.losses}L`} positive={stats.acc >= 60} />
      </div>
